...
static step_motor_err_t
step_motor_device_initialize(step_motor_t const *motor) {
...
static step_motor_err_t
step_motor_device_deinitialize(step_motor_t const *motor) {
...
static step_motor_err_t
step_motor_device_set_frequency(step_motor_t const *motor, uint32_t frequency) {
...
static step_motor_err_t
step_motor_device_set_direction(step_motor_t const *motor,
...
static step_motor_err_t
step_motor_set_direction(step_motor_t *motor,
                         step_motor_direction_t direction) {
...
static inline step_motor_direction_t
step_motor_speed_to_direction(step_motor_t const *motor, float32_t speed) {
  if (fabsf(speed) < motor->config.min_speed /*||
...
step_motor_position_to_step_count(step_motor_t const *motor,
                                  float32_t position) {
  assert(motor->config.step_change > 0.0F);
...
step_motor_step_count_to_position(step_motor_t const *motor,
                                  int64_t step_count) {
  assert(motor->config.step_change > 0.0F);
